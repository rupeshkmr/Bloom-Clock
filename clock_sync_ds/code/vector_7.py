...
def event(pid,counter,eid):
    ...
    data = {eid:counter}
    with open('vector_poset_7.txt', 'a') as outfile:
...
def process_one(pipe13,pipe14):
    ...
    counter = send_message(pipe13,pid,counter,'b')
    counter = recv_message(pipe14,pid,counter,'c')
    counter = send_message(pipe14,pid,counter,'d')
    counter = recv_message(pipe14,pid,counter,'e')
...
def process_three(pipe31,pipe32):
    pid = 2
    counter = [0,0,0,0]
    counter = send_message(pipe32,pid,counter,'j')
    counter = recv_message(pipe31,pid,counter,'k')
    counter = recv_message(pipe32,pid,counter,'l')
    counter = recv_message(pipe32,pid,counter,'m')

def process_four(pipe41):
    pid = 3
    counter = [0,0,0,0]
    counter = send_message(pipe41,pid,counter,'n')
    counter = event(pid,counter,'o')
    counter = send_message(pipe41,pid,counter,'p')
    counter = recv_message(pipe41,pid,counter,'q')
...
if __name__ == '__main__':
    ...
    poset = {}
    with open('vector_poset_7.txt') as json_file:
