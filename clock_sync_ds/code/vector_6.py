...
def event(pid,counter,eid):
    ...
    data = {eid:counter}
    with open('vector_poset_6.txt', 'a') as outfile:
...
def process_one(pipe13,pipe14):
    ...
    counter = send_message(pipe14,pid,counter,'c') 
...
def process_three(pipe31,pipe32):
    ...
    counter = recv_message(pipe32,pid,counter,'i')
    counter = recv_message(pipe32,pid,counter,'j')

def process_four(pipe41):
    pid = 3
    counter = [0,0,0,0]
    counter = send_message(pipe41,pid,counter,'k')
    counter =event(pid,counter,'l')
...
if __name__ == '__main__':
    ...
    poset = {}
    with open('vector_poset_6.txt') as json_file:
