...
def event(pid,counter,eid):
    ...
    data = {eid:counter}
    with open('vector_poset_3.txt', 'a') as outfile:
...
def process_two(pipe21,pipe23):
    pid = 1
    counter = [0,0,0]
    counter = send_message(pipe21,pid,counter,'c')
    counter = recv_message(pipe23,pid,counter,'d')
...
def process_three(pipe31,pipe32):
    ...
    counter = send_message(pipe32,pid,counter,'g')
...
if __name__ == '__main__':
    ...
    poset = {}
    with open('vector_poset_3.txt') as json_file:
